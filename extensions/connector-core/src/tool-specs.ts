...
export function cotalToolSpecs(config: AgentConfig, source = "connector"): CotalToolSpec[] {
  ...
  const specs: CotalToolSpecDecl[] = [
    ...
    },
    {
      ...
      async run(agent, _config, { name, role, agent: agentType, model, variant, launchOptions, cwd, task }: {
        name: string; role?: string; agent?: string; model?: string; variant?: string;
        launchOptions?: Record<string, unknown>; cwd?: string; task?: string;
      }) {
        try {
          ...
          const d = reply.data as { name?: string; mode?: string; remote?: { id?: string; taskId?: string; status?: string } } | undefined;
          ...
          const receipt = remote?.id
            ? ` — remote job ${remote.id}${remote.taskId ? ` (task ${remote.taskId})` : ""} is ${remote.status ?? "accepted"}`
            : "";
          return ok(`${lead}${mode ? ` (${mode})` : ""}${receipt} — it will appear in the roster shortly.`);
        } catch (e) {
