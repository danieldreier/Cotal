...
| `kvw-{p\|c}-<owner>-<actor>-<watchUid>` | `KV_cotal_{presence\|channels}_<space>` | `$KV.cotal_{presence\|channels}_<space>.>` | trusted-provisioned with fixed `cotal.<space>.kvwatch.{p\|c}.<owner>.<actor>.<watchUid>` push destination; `DeliverPolicy.LastPerSubject`, `AckExplicit`; the agent receives exact INFO/ACK/DELETE plus subscribe to that fixed rail, but no consumer CREATE or pull `MSG.NEXT`. Static/dev provisioning uses the lifecycle UID. A user-auth callout derives a distinct bounded `watchUid` from the validated connection nonce and creates the pair before releasing the broker JWT, so overlapping processes own independent snapshots and deletion rights. Graceful stop deletes that connection's pair; crash leftovers expire by inactivity. |
