...
3. Consumer-create request bodies are not fully ACL-constrainable, so the provisioner pre-creates
   ...
   retain an already-canonical push-bound consumer so a concurrent command or bearer refresh cannot
   reset a live watcher. Every unbound watcher MUST be replaced before bearer release: pending counts
   do not prove a live handoff because ordinary traffic can arrive after an ungraceful predecessor
   exits. Simultaneous ensures for one lifecycle MUST be coalesced, and the fixed consumer name and
   delivery rail let a pending client bind the replacement and receive a fresh current-state snapshot.
