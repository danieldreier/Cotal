...
2. Add the generic manager-owned `RuntimeSpawnContext` (persona, resolved connector/model selectors,
   task, authenticated parent and allocated child lifecycle and role, correlation id) and optional remote launch
...
The external launcher must preserve the manager-issued child lifecycle and manager-allocated role through a
server-side enrollment or immutable job secret. It must not mint a parallel Cotal identity or substitute a
launcher-default role, and no ordinary agent receives the launcher bearer or bootstrap secret.
